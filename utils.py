...
def save_metric(epoch, best_metric):
...
def save_state(config, model, best_metric):
    print('==> Saving model ...')
    ...
    torch.save(state_dict, os.path.join(save_path, 'result_{}.pth'.format(best_metric)))
...
def save_result(config, depths, names, ori_sizes=None):
    ...
    os.makedirs(save_path, exist_ok=True)
    cmap_style = 'plasma'
    # print('generating predictions and their {}_visualizations...'.format(cmap_style))
    for i in range(depths.shape[0]):
        ...
        img = (depth * 1000.0).astype('uint16')
        # Img = Image.fromarray(img)
        # Img.save(filename)
        cv2.imwrite(filename, img)
        # plt.imsave(filename, img, cmap=cmap_style)
...
def init_dataset(config):
    ...
    testset = dataset(**params, mode='test', transform=test_transform)
